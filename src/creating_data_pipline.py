...
MODEL_PATH = MODEL_DIR / "lgbm_unigram_pipeline.joblib"
...
# Cleaning function
# -------------------------------
def data_cleaning_without_emojis(text: str) -> str:
    text = str(text).lower()
...
def full_preprocess(text: str) -> str:
    ...
    text = data_cleaning_without_emojis(text)
    return text


def preprocess_list(text_list):
    return [full_preprocess(t) for t in text_list]
...
def main():
    ...
    # ✅ Full pipeline (preprocess + tfidf + lgbm)
    pipeline = Pipeline(
        [
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        report_path = MODEL_DIR / "classification_report_unigram.txt"
        ...
        plt.title("Confusion Matrix - LGBM + TFIDF Unigram")
        plt.tight_layout()

        cm_path = MODEL_DIR / "confusion_matrix_unigram.png"
